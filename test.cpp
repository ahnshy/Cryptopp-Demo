...
std::string AESEncrypt(std::string& strPlainText, CryptoPP::byte* pKey, CryptoPP::byte* pInitializationVector)
{
	std::string ciphertext;
	std::cout << "Plain Text (" << strPlainText.size() << " bytes)" << std::endl;
	std::cout << strPlainText;
	std::cout << std::endl << std::endl;

	CryptoPP::AES::Encryption aesEncryption(pKey, CryptoPP::AES::DEFAULT_KEYLENGTH);
	CryptoPP::CBC_Mode_ExternalCipher::Encryption cbcEncryption(aesEncryption, pInitializationVector);

	CryptoPP::StreamTransformationFilter stfEncryptor(cbcEncryption, new CryptoPP::StringSink(ciphertext));
	stfEncryptor.Put(reinterpret_cast<const unsigned char*>(strPlainText.c_str()), strPlainText.length());
...
std::string AESDecrypt(std::string& strCipherText, CryptoPP::byte* pKey, CryptoPP::byte* pInitializationVector)
{
	std::string decryptedtext;
	CryptoPP::AES::Decryption aesDecryption(pKey, CryptoPP::AES::DEFAULT_KEYLENGTH);
	CryptoPP::CBC_Mode_ExternalCipher::Decryption cbcDecryption(aesDecryption, pInitializationVector);

	CryptoPP::StreamTransformationFilter stfDecryptor(cbcDecryption, new CryptoPP::StringSink(decryptedtext));
	stfDecryptor.Put(reinterpret_cast<const unsigned char*>(strCipherText.c_str()), strCipherText.size());
